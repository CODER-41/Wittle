...
import { X, Smartphone, CreditCard, ExternalLink } from 'lucide-react'
...
export default function PaymentModal({ invoice, onClose, onSuccess }: Props) {
  ...
  const handleCard = async () => {
    ...
    setLoading(true)
    try {
      const res = await api.post('/payments/card/initiate', {
        invoice_id: invoice.id,
      })
      setCardUrl(res.data.authorization_url)
    } catch (err: any) {
      setError(err.response?.data?.error || 'Failed to initialize card payment')
    } finally {
      setLoading(false)
    }
  }
            ...
            <div className="space-y-3">
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">Send payment request to client's phone</p>
                </div>
              </button>

              <button
                onClick={() => { setMethod('card'); handleCard() }}
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 mt-1">
                  The client will receive an STK push on their phone
                </p>
              </div>
              <button
                onClick={handleMpesa}
